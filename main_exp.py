...
# 【改动1】导入我们刚才新建的指数衰减算法文件
from algo.Geobiasd_HITS_exp import GeobiasedHITS_exp 
...
class FaostatRunner:
    ...
    def run_hits(self, use_geo_bias=True, gamma=1.0):
        print(f"\n=== running {'Geo-biased (Exponential)' if use_geo_bias else 'vanilla'} HITS ===")
        
        if use_geo_bias:
            coords_df = pd.read_csv("./data/countries_coords.csv")
            coords_dict = align_coordinates(self.node_names, coords_df)
            
            # 【改动2】这里使用新的类 GeobiasedHITS_exp
            hits_model = GeobiasedHITS_exp(
                self.adj_matrix, 
                coords_dict, 
                self.node_names, 
                gamma=gamma
...
def main():
    ...
    # 【改动3】为了不覆盖队友的数据，保存的名字加了前缀 'exp_'
    if args.Geo:
        prefix = f"exp_geobiased_gamma_{args.gamma}"
    else:
        prefix = "exp_vanilla"
