...
# 输出路径
OUTPUT_IMG_PATH = os.path.join(os.path.dirname(__file__), "spearman_topK_REWARD_ORI_trend.png")

def run_evaluation():
    # 精准读取原始奖励模型数据
    result_files = [f for f in os.listdir(RESULT_DIR) if f.endswith('.csv') and 'reward_ori_geobiased' in f]
    
    if not result_files:
        print("未找到结果文件，请先运行 main_reward_original.py")
        return
    ...
    sns.lineplot(data=results_df, x='Gamma', y='Spearman_Coef', hue='K_Value', 
                 marker='^', linewidth=2, markersize=8, palette="autumn")
    
    plt.title('Original Reward Model: Impact of Top-K on LPI Correlation', fontsize=15, pad=15)
