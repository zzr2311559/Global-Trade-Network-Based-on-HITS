...
from algo.Geobiasd_HITS_reward_original import GeobiasedHITS_reward_original 
...
class FaostatRunner:
    ...
    def run_hits(self, use_geo_bias=True, gamma=1.0):
        print(f"\n=== running {'Original Reward Distance' if use_geo_bias else 'vanilla'} HITS ===")
        if use_geo_bias:
            coords_df = pd.read_csv("./data/countries_coords.csv")
            coords_dict = align_coordinates(self.node_names, coords_df)
            hits_model = GeobiasedHITS_reward_original(self.adj_matrix, coords_dict, self.node_names, gamma=gamma)
        else:
...
def main():
    ...
    prefix = f"reward_ori_geobiased_gamma_{args.gamma}" if args.Geo else "reward_ori_vanilla"
